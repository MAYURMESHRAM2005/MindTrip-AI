...
const MODE_META = {
  ...
  car: { icon: Car, labelKey: 'Car / Road', color: 'bg-emerald-50 text-emerald-600 dark:bg-emerald-950 dark:text-emerald-400' },
};
  ...
  return (
    <Card className="mb-6">
      <CardHeader
        ...
        subtitle={t('Mode-matched travel plan with estimated costs')}
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-xl bg-slate-50 p-4 dark:bg-slate-900/40">
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">{t('Estimated fare')}</p>
          ...
          <p className="text-[11px] text-slate-400">{t('per direction from transport budget')}</p>
        </div>
        {plan.duration != null && (
          <div className="rounded-xl bg-slate-50 p-4 dark:bg-slate-900/40">
        ...
        {plan.tollCost != null && (
          <div className="rounded-xl bg-slate-50 p-4 dark:bg-slate-900/40">
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">{t('Toll cost')}</p>
            <p className="mt-1 inline-flex items-center gap-1 text-xl font-extrabold text-slate-900 dark:text-white">
      ...
      {plan.suggestions?.length > 0 && (
        <div className="mt-4">
          <p className="mb-2 text-xs font-bold uppercase tracking-wide text-slate-400">{t('Options')}</p>
